...
extern struct dc_motor dc[dc_len];
extern struct dc_motor gp_driver;
extern struct stepper stp[stp_len];

extern uint32_t adc1_buf[]; //AS5600 adc buffer
extern uint32_t adc2_buf[]; //AS5600 adc buffer
/* USER CODE END PV */
...
int tim7_handler(void){
	...
	//set pulse of the gp driver;
	gp_driver.pulse = clamp_float(gp_driver.pulse, -1.0, 1.0);
	dc_set_pulse(&gp_driver);
	//now steppers
	for (c = 0; c < stp_len; ++c) {
